...
; *******************************************************************
    ...
    mov di, play_string
    call os_string_compare  ; "Jugar" entered ?
    ;jc near play_file
...
; EXTENSION DE LOS ARCHIVOS, PARAMETROS Y COMANDOS POR TECLADOS USADOS ARRIBA 
	...
	invalid_msg		    db 'No existe tipo de comando o programa, vuelva  a intentarlo', 13, 10, 0
	nofilename_msg		db 'No se encuentra nombre del archivo', 13, 10, 0
	notfound_msg		db 'Archivo no encontrado', 13, 10, 0
	writefail_msg		db 'No se pudo escribir el archivo. Compruebe si esta protegido contra escitura', 13, 10, 0
	...
	ver_string		db 'VER', 0
	cat_string		db 'CAT', 0
	del_string		db 'ELIMINAR', 0
	ren_string		db 'REN', 0
	copy_string		db 'COPIAR', 0
	size_string		db 'TAMANO', 0
    play_string     db 'JUGAR',0

	kern_file_string	db 'KERNEL', 0
	kern_warn_msg		db 'No se puede ejecutar archivo Kernel', 13, 10, 0
